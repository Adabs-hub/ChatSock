    ...
    if (Network::Initialize())
    {
		...
		while (1)
		{
			if (!server_located)
			{
				...
				if (udp.create(SOCK_DGRAM, false) == PResult::P_NotYetImplemented)
				{		
				...
				if (udp.Listener(IPEndPoint("0.0.0.0", BROADCAST_PORT), &available_server_addr) == PResult::P_Success)
				{
			...
			{
				Socket socket;
				if (socket.create(SOCK_STREAM, false) == PResult::P_Success)
				{

					std::cout << "socket created successfully" << std::endl;
					if (socket.Connect(&available_server_addr) == PResult::P_Success)
					{
